...
//	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
   ...
   gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, outwidth, outheight,
...
studiohdr_t *StudioModel::LoadModel( char *modelname )
{
   ...
   pakinfo_t *pakinfo = NULL;
	...
	// load the model
   if ((fp = OpenMODFile(modelname)) != NULL)
	{
      fseek( fp, 0, SEEK_END );
      size = ftell( fp );
      fseek( fp, 0, SEEK_SET );
   ...
   else if (SearchPakFilename(modelname, &pakconfig, &pakinfo))
   {
      buffer = malloc(pakinfo->file_length);

      P_ReadPakItem(pakconfig->pakhandle, pakinfo, buffer);
   }
   else
   {
      return NULL;
...
studioseqhdr_t *StudioModel::LoadDemandSequences( char *modelname )
{
   ...
   return NULL;  // won't ever get here since Error will exit
}


bool StudioModel::Init( char *modelname )
{
   m_origin[0] = 0;
   m_origin[1] = 0;
   m_origin[2] = 0;
   ...
   m_skinnum = 0;
	...
	if ((m_pstudiohdr = LoadModel( modelname )) == NULL)
      return FALSE;
